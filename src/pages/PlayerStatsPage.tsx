...
import { Download, FileSpreadsheet, Plus, Search, Save, Trash2, Upload, Users, X } from 'lucide-react';
...
const emptyForm: PlayerForm = {
  ...
  assists_per_match: 0, confidence: 0, goal_contribution_pm: 0, goal_contribution: 0, season: '2025-2026',
};
...
function PlayerStatsPage() {
  ...
  const filtered = useMemo(() => players.filter((player) => `${player.name} ${player.position || ''}`.toLowerCase().includes(search.toLowerCase()) && (position === 'All positions' || player.position === position)), [players, position, search]);
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm min-w-[1100px]"><thead><tr className="border-b border-slate-800 text-xs text-slate-500 uppercase tracking-wider">{columns.map((column) => <th key={column.key} className={column.key === 'name' || column.key === 'position' ? 'text-left px-5 py-4' : 'text-right px-4 py-4'}>{column.label}</th>)}<th className="px-5 py-4" /></tr></thead><tbody>{filtered.map((player) => <tr key={player.id} className="border-b border-slate-800/70 last:border-0 hover:bg-slate-800/30"><td className="px-5 py-3.5 font-medium text-slate-200">{player.name}</td><td className="px-5 py-3.5 text-slate-400">{player.position || '—'}</td>{columns.slice(2).map((column) => <td key={column.key} className="px-4 py-3.5 text-right text-slate-300">{column.decimals ? Number(player[column.key] || 0).toFixed(3) : player[column.key]}</td>)}<td className="px-5 py-3.5 text-right"><button onClick={() => openEdit(player)} className="text-xs text-slate-500 hover:text-white">Edit</button></td></tr>)}</tbody></table></div>{!filtered.length && <div className="py-16 text-center"><Users className="w-8 h-8 text-slate-700 mx-auto mb-3" /><p className="text-slate-500 text-sm">No players found</p></div>}<div className="px-5 py-3 border-t border-slate-800 text-xs text-slate-600">Showing {filtered.length} of {players.length} players</div></div>
